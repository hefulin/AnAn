"use strict";
/**
 * Created by rocky on 16/7/1.
 */

var _createClass = function () { function defineProperties(target, props) { for (var i = 0; i < props.length; i++) { var descriptor = props[i]; descriptor.enumerable = descriptor.enumerable || false; descriptor.configurable = true; if ("value" in descriptor) descriptor.writable = true; Object.defineProperty(target, descriptor.key, descriptor); } } return function (Constructor, protoProps, staticProps) { if (protoProps) defineProperties(Constructor.prototype, protoProps); if (staticProps) defineProperties(Constructor, staticProps); return Constructor; }; }();

function _classCallCheck(instance, Constructor) { if (!(instance instanceof Constructor)) { throw new TypeError("Cannot call a class as a function"); } }

var svgicons2svgfont = require('svgicons2svgfont');
var svg2ttf = require('svg2ttf');
var ttf2eot = require('ttf2eot');
var ttf2woff = require('ttf2woff');
var ttf2woff2 = require('ttf2woff2');
var jsZip = require('jszip');
var stringDecoder = require('string_decoder').StringDecoder;
var _ = require("underscore");
var stream = require("stream").PassThrough;
var fs = require("fs");
var utils = require("./utils");
var path = require("path");
var cleanCss = require("clean-css");
/**
 * 运行生成字体文件
 */

var fontBunder = function () {

    /**
     * 字体打包压缩
     */
    function fontBunder() {
        _classCallCheck(this, fontBunder);

        this.cleanCss = true;
        this.isZip = true;
        this.hasDemo = true;
        this.viewBoxSize = true;
        this.fontOptions = null;
    }

    /**
     * setHasDemo
     * @param boolean bool bool为 输出demo文件，不输出
     * @return self
     */


    _createClass(fontBunder, [{
        key: 'setHasDemo',
        value: function setHasDemo(bool) {
            this.hasDemo = bool === false ? false : true;
            return this;
        }

        /**
         * setIsZip
         * @param boolean bool bool 为 输出zip文件流，否则输出若干个文件内容
         * @return self
         */

    }, {
        key: 'setIsZip',
        value: function setIsZip(bool) {
            this.isZip = bool === false ? false : true;
            return this;
        }

        /**
         * setCleanCss
         * @param boolean bool bool 是否输出美化后的css, 默认为true
         * @return self
         */

    }, {
        key: 'setCleanCss',
        value: function setCleanCss(bool) {
            this.cleanCss = bool === false ? false : true;
            return this;
        }

        /**
         * 格式化画布大小
         */

    }, {
        key: 'resetViewBoxSize',
        value: function resetViewBoxSize(bool) {
            this.viewBoxSize = bool === false ? false : true;
            return this;
        }

        /**
         * 格式化路径
         * @param dir
         * @param filePath
         */

    }, {
        key: 'formatPath',
        value: function formatPath(dir, filePath) {
            dir = dir || "";
            filePath = filePath || "";
            return path.normalize((dir + path).replace("//", "/"));
        }
        /**
         * 格式化字体参数
         * @param object options 字体配置
         * @return {*}
         */

    }, {
        key: 'formatFontOptions',
        value: function formatFontOptions(options) {
            var def = {
                fontName: "iconfonts",
                normalize: true, //所有字体标准输出
                fontHeight: 1000, //字体高度200
                centerHorizontally: true, //字体居中生成
                prependUnicode: false, //自动生成unicode
                startUnicode: 0xEA01, //自动开始编号
                className: "iconfont", //class样式名称,
                iconPrefix: "icon" //生成的icon css class名称
            };
            options = _.extend({}, def, options);
            _.forEach(options, function (val, key) {
                val === null && delete options[key];
            });
            options.fontName = options.fontName || def.fontName;
            options.className = options.className || def.className;
            options.iconPrefix = options.iconPrefix || def.iconPrefix;
            this.fontOptions = options;
        }

        /**
         * 写入需要编辑的svg配置
         * @param array fonts
         * [
         *  {
         *      file : "filepath",//文件所在路径
         *      content : "content", //字体文件内容
         *      name : "name", //字体名称昵称
         *      clasname : "classname", //文件自定义classname
         *      unicode : "ea00", //自定义字体编码
         *  }
         * ]
         * @param object fontOptions
         * @return boolean
         */

    }, {
        key: 'formatFonts',
        value: function formatFonts(fonts) {
            var _this = this;
            if (!_.isArray(fonts)) {
                return [];
            }
            fonts = fonts.map(function (font) {
                if (_.isEmpty(font.file) && _.isEmpty(font.content)) {
                    return false;
                }
                var unicode = null;
                if (!!_this.fontOptions.prependUnicode) {
                    unicode = _this.fontOptions.startUnicode++;
                } else {
                    unicode = utils.hex2Int(font.unicode);
                    if (false === unicode) {
                        return false;
                    }
                }
                var code = utils.int2Hex(unicode);
                var tmp = {};
                tmp.file = font.file ? path.normalize(font.file) : "";
                tmp.content = font.content || null;
                tmp.name = code;
                tmp.classname = font.classname || code;
                tmp.unicode = [String.fromCharCode(unicode)];
                tmp.code = code;
                return _.extend({}, font, tmp);
            });
            return fonts.filter(function (font) {
                return font !== false;
            });
        }

        /**
         * 运行生成文件
         * @param string files 文件格式集合,可以查看formatFonts注释
         * @param fontOptions 字体配置
         */

    }, {
        key: 'generate',
        value: function generate(fonts, fontOptions, callback) {
            var _this = this;
            if (_.isFunction(fontOptions)) {
                callback = fontOptions;
                fontOptions = {};
            }
            fontOptions = this.formatFontOptions(fontOptions);
            fonts = this.formatFonts(fonts);
            if (fonts.length < 1) {
                callback("files is empty || fonts is a invalid data");
                return;
            }
            var iconStreams = [];
            _.forEach(fonts, function (font) {
                var tmp = _this.newIconStream(font);
                if (tmp !== null) {
                    iconStreams.push(tmp);
                }
            });
            var data = [];
            if (this.hasDemo) {
                data = this.fontDemo(fonts);
            }
            this.makeSVG(iconStreams, function (content) {
                //svg
                data.push({
                    name: _this.fontOptions.fontName + ".svg",
                    content: content
                });
                //ttf
                var ttfFont = _this.makeTTF(content);
                var eotFont = _this.makeEOT(ttfFont.buffer);
                var woffFont = _this.makeWOFF(ttfFont.buffer);
                var ttfBuffer = new Buffer(ttfFont.buffer);
                var ttfUint8Array = new Uint8Array(ttfBuffer);
                data.push({
                    name: _this.fontOptions.fontName + ".ttf",
                    content: ttfFont.buffer,
                    buffer2Content: new Buffer(ttfFont.buffer)
                });
                //eot
                data.push({
                    name: _this.fontOptions.fontName + ".eot",
                    content: eotFont.buffer,
                    buffer2Content: new Buffer(_this.makeEOT(ttfUint8Array).buffer)
                });
                //woff
                data.push({
                    name: _this.fontOptions.fontName + ".woff",
                    content: woffFont.buffer,
                    buffer2Content: new Buffer(_this.makeWOFF(ttfUint8Array).buffer)
                });
                //woff2
                data.push({
                    name: _this.fontOptions.fontName + ".woff2",
                    content: _this.makeWOFF2(ttfFont.buffer),
                    buffer2Content: new Buffer(_this.makeWOFF2(ttfUint8Array).buffer)
                });
                if (_this.isZip) {
                    var zip = new jsZip();
                    _.forEach(data, function (row) {
                        zip.file(row.name, row.content);
                    });
                    callback(zip.generate({ type: "nodebuffer", compression: 'DEFLATE' }));
                    zip = null;
                } else {
                    data = _.map(data, function (row) {
                        row.content = row.buffer2Content || row.content;
                        return row;
                    });
                    _.forEach(fonts, function (font, key) {
                        delete fonts[key].content;
                        delete fonts[key].file;
                        delete fonts[key].unicode;
                    });
                    callback(data, fonts);
                    data = null;
                    fonts = null;
                }
            });
        }

        /**
         *
         * @param object file 文件字体格式
         * @return object
         */

    }, {
        key: 'newIconStream',
        value: function newIconStream(font) {
            var iconStream = new stream();
            var content = "";
            try {
                if (_.isString(font.content) && font.content != "") {
                    content = font.content;
                } else {
                    content = fs.readFileSync(font.file, "utf8");
                }
                //fixed bug 修复当画布大小不一致时导致的何必错误
                if (this.viewBoxSize) {
                    var viewBoxRE = /viewBox=[^ ]+\s+[^ "']+\s+([^ "']+)\s+([^ "']+)/;
                    var widthRE = /<svg [^>]+width=([0-9\."]+)[^>]+>/;
                    var heightRe = /<svg [^>]+height=([0-9\."]+)[^>]+>/;
                    var w = void 0;
                    var h = void 0;
                    //重新设置画布
                    content.replace(viewBoxRE, function () {
                        w = RegExp.$1;
                        h = RegExp.$2;
                    });
                    //fixbug 调整错误
                    if (w && h) {
                        content = content.replace(widthRE, function (v) {
                            return v.replace(/width="[0-9\.]+(px)?"/, "width=\"" + w + "px\"");
                        });
                        content = content.replace(heightRe, function (v) {
                            return v.replace(/height="[0-9\.]+(px)?"/, "height=\"" + h + "px\"");
                        });
                    }
                }
                iconStream.write(content, "utf8");
                iconStream.end();
            } catch (e) {
                iconStream.end();
                return null;
            }
            iconStream.metadata = {
                unicode: font.unicode,
                name: font.name
            };
            return iconStream;
        }

        /**
         * SVG字体数据
         * @param iconStreams
         * @param callback
         */

    }, {
        key: 'makeSVG',
        value: function makeSVG(iconStreams, callback) {
            var parts = [];
            var decoder = new stringDecoder('utf8');
            var fontStream = svgicons2svgfont(this.fontOptions);
            fontStream.on('data', function (chunk) {
                parts.push(decoder.write(chunk));
            });
            fontStream.on('finish', function () {
                callback(parts.join(''));
            });
            iconStreams.forEach(fontStream.write.bind(fontStream));
            fontStream.end();
        }

        /**
         * TTF字体数据
         * @param svgFont
         * @return
         */

    }, {
        key: 'makeTTF',
        value: function makeTTF(svgFont) {
            return svg2ttf(svgFont);
        }

        /**
         * EOT字体数据
         * @param ttfFontBuffer
         * @return {*}
         */

    }, {
        key: 'makeEOT',
        value: function makeEOT(ttfFontBuffer) {
            return ttf2eot(ttfFontBuffer);
        }

        /**
         * WOFF字体文件
         * @param ttfFontBuffer
         * @return {*}
         */

    }, {
        key: 'makeWOFF',
        value: function makeWOFF(ttfFontBuffer) {
            return ttf2woff(new Uint8Array(ttfFontBuffer.buffer));
        }

        /**
         * WOFF字体文件
         * @param ttfFontBuffer
         * @return {Uint8Array}
         */

    }, {
        key: 'makeWOFF2',
        value: function makeWOFF2(ttfFontBuffer) {
            ttfFontBuffer = new Uint8Array(ttfFontBuffer);
            var buf = new Buffer(ttfFontBuffer.length);
            var i = 0;
            var j = 0;
            for (i = 0, j = ttfFontBuffer.length; i < j; i++) {
                buf.writeUInt8(ttfFontBuffer[i], i);
            }
            buf = ttf2woff2(buf);
            var woff2FontBuffer = new Uint8Array(buf.length);
            for (i = 0, j = buf.length; i < j; i++) {
                woff2FontBuffer[i] = buf.readUInt8(i);
            }
            return woff2FontBuffer;
        }

        /**
         * demo例子
         */

    }, {
        key: 'fontDemo',
        value: function fontDemo(fonts) {
            var _this = this;
            var p = path.normalize(__dirname + "/../iconsTmp/");
            var iconsTemplate = ["demo.css", "demo.html", "iconfont.css"];
            var result = [];
            var annotation = '/**\n * use-iconfonts http://github.com/rockywu/use-iconfonts\n * @author rockywu <wjl19890427@hotmail.com>\n */\n';
            _.forEach(iconsTemplate, function (fileName) {
                var content = _this.template(fs.readFileSync(p + fileName, "utf8"), {
                    fonts: fonts,
                    fontName: _this.fontOptions.fontName,
                    className: _this.fontOptions.className,
                    iconPrefix: _this.fontOptions.iconPrefix
                });
                if (_this.cleanCss && /\.css$/.test(fileName)) {
                    content = new cleanCss({
                        compatibility: "ie7"
                    }).minify(content).styles;
                }
                //fix v1.9.5 bug, output error message into the demo.html file
                result.push({
                    name: fileName,
                    content: (fileName.indexOf(".css") >= 0 ? annotation : "") + content
                });
            });
            return result;
        }

        /**
         * 运行模板替换
         * @param string content
         * @param object data
         * @return string
         */

    }, {
        key: 'template',
        value: function template(content, data) {
            data = data || {};
            return _.template(content.toString())(data);
        }
    }]);

    return fontBunder;
}();

exports = module.exports = fontBunder;